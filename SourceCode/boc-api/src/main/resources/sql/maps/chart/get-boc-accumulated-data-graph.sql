...
region_info as
 ...
 graph_data as (select a.regionCode, a.regionName,
    case when :p_region_level = 0 then listagg(nvl(spc.actual_acc, -9999), '; ') WITHIN GROUP (ORDER BY a.range_start_date)
        when :p_region_level = 1 then listagg(nvl(pro.actual_acc, -9999), '; ') WITHIN GROUP (ORDER BY a.range_start_date)
        else listagg(nvl(dis.actual_acc, -9999), '; ') WITHIN GROUP (ORDER BY a.range_start_date) end actual,
  ...
  from date_control dc
          inner join (select bs1.day_id, 
            ...
            and to_char(bs.day_id, 'YYYY') = ta.target_month
