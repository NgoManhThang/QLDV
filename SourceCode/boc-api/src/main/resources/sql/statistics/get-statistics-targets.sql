...
select 
	dat_total targetD,
	(select count(*) from kpi_list) totalTargetD,
	m1_total targetM1,
	(select count(*) from kpi_list) totalTargetM1,
	m2_total targetM2,
	(select count(*) from kpi_list) totalTargetM2,
	m3_total targetM3,
	(select count(*) from kpi_list) totalTargetM3
  from(
select *
  from(
select kpicode, result
    from result_data
    where to_char(day_id,'MM/YYYY') = to_char(add_months(f_boc_date(kpiCode), 0),'MM/YYYY')
   )
   pivot (count(kpicode) as total for (result) in ('D' dat, 'M1' m1, 'M2' m2, 'M3' m3)))